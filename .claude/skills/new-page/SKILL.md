...
את **כל** הרשומות לכתובת אחת ומוחקת אותן מהאינדקס. במסלול הזה היא
מזריקה `noindex,follow` במקום, אבל **רק** כשהמסד ענה במפורש שאין רשומה:
`docs/social-preview.md`.

**וכל קישור לדף החדש — ואליו — נכתב בלי הסיומת:**

```html
<a href="/my-page">…</a>     <!-- ולא my-page.html -->
<a href="/">…</a>            <!-- דף הבית -->
```

זה חל גם על כתובת שנבנית ב-JS (`'/property?id=' + id`) ועל קישור
ש-Edge Function שולחת בוואטסאפ או במייל. `href="my-page.html"` עובד,
נראה תקין ומשלם הפניה 301 בכל לחיצה - ובשביל גוגל זו כתובת שכל מה שיש
בה הוא הפניה, כלומר הדוח "הדף מפנה לכתובת אתר אחרת". הבדיקה למטה חוסמת
את זה.
