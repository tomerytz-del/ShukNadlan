...
| "השאירו פרטים לקבלת פגישה בחינם" | כפתור זהב מלא, ראו למטה |
...
### "השאירו פרטים לקבלת פגישה בחינם" — ולמי הליד הולך

התיבה עונה על השאלה שנשאלת מיד אחרי שרואים מה AI עשה לנכס של מישהו אחר:
"ומה עם שלי?". היא **אינה טופס** ואינה שולחת דבר: היא כפתור שקורא ל-`onLead`
של הדף. היא זהובה מלאה ולא מסגרת זהב על ספיר — מסגרת על רקע כהה נקראת
כהודעה ולא כפעולה, וזו הפעולה שכל התיבה מובילה אליה.

בדף הנכס `onLead` מקפיץ את טופס **"תיאום ביקור בנכס"** שכבר בעמוד, ממלא לו
הודעה מוכנה ומעביר אליו את המיקוד. הטופס ההוא שולח ל-`property-inquiry-intake`,
ולכן:

* הליד משויך ל-`property.agent_id` — הסוכן/ת שהנכס שלו/ה;
* והוא נפתח **לפי התוכנית של אותו סוכן/ת**: ‏Mid ו-Premium מקבלים אותו
  `unlocked` מיד וללא עלות, ‏Free מקבל/ת `masked` ופותח/ת דרך ה-CRM.

זו הסיבה שהכפתור לא שולח בעצמו. טופס שני שהיה פונה לאותה Edge Function היה
משכפל את הלוגיקה בלי להוסיף כלום, וטופס שהיה פונה למקום אחר היה מנתק את הליד
מהתוכנית של הסוכן/ת. אותו כלל תקף לכל דף שיאמץ את הרכיב: הרכיב מציג, הדף
מנתב — ובדף סוכן/ת או משרד הניתוב הוא `agent_slug`/`agency_slug` דרך
`owner-lead-intake` (ראו `docs/lead-routing.md`).

ההודעה המוכנה נדרסת רק כשהיא ריקה או כשהיא זו שהדף כתב בעצמו (המשפט עם מספר
המודעה). מי שהתחיל/ה לכתוב הודעה משלו/ה לא מאבד/ת אותה.
