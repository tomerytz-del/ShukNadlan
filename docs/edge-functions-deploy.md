...
לכן `supabase/config.toml` מחזיק `verify_jwt` מפורש **לכל הפונקציות שבריפו**
(57 נכון להיום), והערכים הועתקו אחד לאחד ממה שפרוס בפועל ולא נכתבו מחדש.

על הכיסוי הזה שומרות שתי בדיקות, ובכוונה בשני זמנים שונים:

| מתי | מה | איפה |
| --- | --- | --- |
| על ה-PR | גוש חסר, גוש בלי `verify_jwt`, רשומה בלי תיקייה | `scripts/check_edge_config.py` (workflow `edge_config.yml`) |
| ב-push ל-main | גוש חסר, לפני שהפריסה מתחילה | שלב בתוך `supabase_functions.yml` |

השנייה עוצרת את הפריסה, אבל בתור workflow אדום על `main` — כלומר אחרי
המיזוג. הראשונה נועדה שלא נגיע לשם. אפשר להריץ אותה ידנית:
...
הפריסה **אינה** בודקת טיפוסים - היא מאגדת ופורסת. לכן
`.github/workflows/edge_types.yml` מריץ `deno check` על כל פונקציה בכל PR
שנוגע ב-`supabase/functions`:

```sh
python3 scripts/check_edge_types.py            # כל הפונקציות
python3 scripts/check_edge_types.py lead-claim # אחת
python3 scripts/check_edge_types.py --update   # כתיבת הרף מחדש
```

**זה מחגר, לא שער.** כשהבדיקה נכתבה (26.9.2026) היו 46 פונקציות נקיות
ו-12 עם 258 שגיאות, רובן מ-`_shared/afula-planning.ts` שנכתב כ-JS ונספר בכל
מי שמייבא אותו. בדיקה שנכשלת מהיום הראשון היא בדיקה שמכבים, ולכן
`scripts/edge_types_baseline.json` רושם את המצב הקיים:

| מצב | תוצאה |
| --- | --- |
| פונקציה שאינה ברף | חייבת 0. כך נולדת כל פונקציה חדשה |
| יותר שגיאות מהרף | נכשל, עם הפלט של `deno check` |
| פחות שגיאות מהרף | עובר, ומדפיס לעדכן את הרף באותו PR |
| `deno check` לא רץ עד הסוף (רשת, תלות) | נכשל - "לא רץ" אינו "אפס שגיאות" |

**גרסת Deno נעולה ב-workflow** (‏`v2.9.7`), כי מספר השגיאות תלוי בגרסת
TypeScript שבתוכה. עלייה שאינה קשורה ל-diff היא קודם כול שאלה על גרסה -
גם `jsr:@supabase/supabase-js@2` אינה נעולה לגרסה מדויקת.
