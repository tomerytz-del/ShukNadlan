...
## הכתיבה מחדש של הטקסטים הישנים (‏3 בספטמבר 2026)

הפוסט הראשון שיצא לדף חשף שהמגבלות של הפרומפט חלות **רק על טקסט
ש-Claude כותב**. נכס שהגיע עם `marketing_description` קיים עובר כמו שהוא,
ולכן הפוסט הראשון יצא עם "להשקעה בטוחה" ועם מספר טלפון בגוף הטקסט — שני
דברים שהפרומפט אוסר במפורש, ושהגיעו מטקסט שנכתב לפני שהמנגנון הזה קיים.

סריקה של 60 הנכסים הפעילים מצאה **26** עם מספר טלפון בטקסט ו-**5** עם
הבטחת תשואה. כולם נכתבו מחדש ידנית, מהעובדות בלבד — לא מהטקסט הישן, כדי
לא לגרור הלאה טענות שאיש לא אימת (למשל "6.6 אחוז תשואה" על נכס 1067,
ו-350 מ״ר שדווחו כשטח בנוי בעוד שהם שטח המגרש בנכס 1079).

**‏`force` דרך ה-Edge Function לא היה עושה את העבודה הזו** — הוא עובר
דרך `queue_property_publication` ואז `handle`, כלומר כתיבה מחדש **ופרסום**.
הרצתו על 26 נכסים הייתה מציפה את הדף ב-26 פוסטים, בדיוק מה שסעיף "הנכסים
שכבר במערכת" נמנע ממנו. אין כרגע מסלול שכותב טקסט בלי לפרסם; אם יידרש
שוב, זו תוספת של דגל `text_only` שמדלג על התור ועל הפרסום.

הטקסט המקורי שמור:

```sql
-- מה גובה
select * from marketing_description_backup_20260903;

-- שחזור נכס יחיד
update properties p
   set marketing_description = b.marketing_description,
       post_text = b.post_text
  from marketing_description_backup_20260903 b
 where b.property_id = p.id and p.listing_number = <מספר המודעה>;
```

באותה הזדמנות נכתב גם `post_text` קצר לכל 26 הנכסים. בלעדיו הפתיח של
הפוסט הוא התיאור המלא, והמחיר והמ״ר מופיעים פעמיים — פעם בטקסט ופעם
בשורות ה-`💰`/`🏠` שהמערכת מוסיפה.

### הבדיקה שנשארה פתוחה

‏Custom webhook ב-Make מחזיר 200 **מיד**, לפני שהתרחיש רץ. הקוד מסמן
`posted` על סמך התשובה הזו, ולכן `posted` בטבלה אינו הוכחה שהפוסט קיים
בדף. מודול `Webhooks → Webhook response` בסוף התרחיש מחזיר את `post_id`
האמיתי והופך את הסימון לאמת — כל עוד הוא חסר, `post_id` ו-`post_url`
נשארים `null` וצריך לאמת בעין.
