...
| `supabase/migrations/20260907090000_property_publication_manual.sql` | הפרסום החוזר היזום: `origin`, החסם החודשי, וההרשאה לסוכן/ת |
| `supabase/functions/property-marketing-publish/index.ts` | השרת: כותב את הטקסט ב-Claude ומפרסם |
| `crm.html` → כרטיס הנכס | הכפתור "📘 פרסם בפייסבוק" ותגית מצב הפרסום |
| `docs/facebook-auto-publish.md` | הקובץ הזה |

שתי המיגרציות אידמפוטנטיות.

## המסלול, מקצה לקצה

```
נכס נשמר כ-active                    כפתור "פרסם בפייסבוק" בכרטיס
        │  טריגר                              │  request_manual_publication
        │  properties_queue_publication        │  (בעלות · פעיל · פעם ב-30 יום)
        ▼                                      ▼
property_publications — auto: +20 דקות · manual: מיד
...
ארבע שכבות: אינדקס `unique` חלקי על `(property_id, channel) where origin =
'auto'` (הפרסום האוטומטי קורה בדיוק פעם אחת בחיי הנכס — גם אם הוא נמכר וחזר
לשוק), `claim_property_publication` שנועלת את השורה לפני הפרסום מפני שתי
הרצות חופפות, שורה אחת לכל נכס בכל הרצה של השרת, וסטטוס `posted` שמוציא את
השורה מהתור. הפרסום החוזר היזום הוא שורה חדשה ומכוונת — והוא חסום ל-30 יום.
...
| `facebook_republish_min_days` | 30 | כמה ימים מהפוסט האחרון לפני שהכפתור בכרטיס נפתח שוב |
...
בכרטיס הנכס ב-CRM, לצד "הקפצה חינם", יושב **📘 פרסם בפייסבוק**. הוא מופיע
רק כשהפרסום באמת אפשרי; בשאר המצבים התגית שבראש הכרטיס מספרת מה קורה:

| מצב הנכס | מה רואים |
| --- | --- |
| טרם פורסם | כפתור **📘 פרסם בפייסבוק** |
| בתור (אוטומטי או יזום) | תגית **📘 בתור לפרסום**, בלי כפתור |
| פורסם לפני פחות מ-30 יום | תגית **📘 פורסם ‹תאריך›** — קישור לפוסט עצמו — ובה גם המועד שממנו אפשר שוב |
| פורסם לפני 30 יום ומעלה | תגית הפרסום + כפתור **📘 פרסום חוזר בפייסבוק** |

הכפתור לא מפרסם בעצמו. הוא קורא ל-`request_manual_publication`, שמכניסה
שורה לתור, ומיד אחר כך לשרת הפרסום כדי שהפוסט יצא עכשיו ולא בעוד חמש
דקות. אם הקריאה השנייה נכשלת — ה-cron ירים את השורה בהרצה הבאה, ולכן
ההודעה לסוכן/ת היא "נשלח לפרסום" ולא הבטחה לקישור.

### הכללים, וכולם במסד

`request_manual_publication` היא הסמכות היחידה. הכפתור ב-CRM רק משקף אותה,
כך שגם קריאה ישירה ל-API לא תעקוף אותו:

| כלל | סיבה שמוחזרת |
| --- | --- |
| הנכס שלי, או נכס במשרד שאני מנהל/ת | `not_allowed` |
| הנכס פעיל | `not_active` |
| המנגנון דלוק | `disabled` |
| אין כבר פרסום בתור על הנכס | `already_queued` |
| עברו `facebook_republish_min_days` מהפוסט האחרון | `too_soon` + `next_allowed_at` |

**החסם נספר על פני כל הפוסטים, אוטומטי כידני.** נכס שנכנס אתמול וקיבל את
הפוסט האוטומטי שלו לא יקבל פוסט שני היום — הפרסום החוזר הראשון שלו יהיה
בעוד חודש.

### למה פוסט חוזר נכתב מחדש

פוסט זהה שחוזר כעבור חודש נראה לגולש/ת כמו ספאם, ופייסבוק מדכא תוכן שחוזר
מילה במילה. לכן על שורת `origin = 'manual'` השרת מבקש מ-Claude פתיח חדש —
זווית אחרת, אותן עובדות. `marketing_description` ו-`post_text` של הנכס
**לא** משתנים: הסוכן/ת פותח/ת את הכרטיס ומוצא/ת שם את הנוסח שהוא/היא
מכיר/ה. הנוסח שיצא בפועל נשמר על שורת הפרסום, כרגיל.

אם הניסוח מחדש נכשל (Anthropic לא זמין) הפוסט יוצא בנוסח הקיים ולא נופל.
...
נכס ותיק לא נשאר מאחור: מכיוון שאין עליו שורת `posted`, הכפתור בכרטיס שלו
פתוח כבר עכשיו — הסוכן/ת מפרסם/ת אותו בלחיצה, מתי שנוח, בלי שכולם ייצאו
באותו יום.
...
> **תרחיש חדש, לא שינוי של הקיים.** אם כבר רץ אצלכם תרחיש שמפרסם לדף, אל
> תשנו אותו: הוא מופעל ממקור אחר (טופס, גיליון, RSS) ויש לו מבנה נתונים
> משלו. תרחיש נפרד ל-webhook הזה משאיר את שניהם ניתנים לתיקון בנפרד, ואת
> שדה `message` מוכן לפרסום כמו שהוא.
...
סוכן/ת לא צריך/ה שום פקודה — הכפתור בכרטיס הנכס עושה בדיוק את זה, בכפוף
לחסם החודשי. הפקודות כאן הן המסלול של מנהל/ת הפלטפורמה, והן היחידות
שעוקפות את החסם.
...
- **אין פאנל ניהול ליומן הפרסומים.** הכפתור בכרטיס מכסה את הסוכן/ת;
  מנהל/ת הפלטפורמה עדיין קורא/ת את היומן ב-SQL (ראו "מעקב").
