/* ============================================================================
   ...
   - ‏500 מ-getSearchResultData (קרה ל-10 מ-144 באותה ריצה): ניסיון חוזר,
     ואם הוא נכשל שוב ושוב - מרכז בלי גבול, לא כישלון של הסריקה
...
function run(opts){
  const calls = { layer: 0, verify: 0, rd: 0, search: [] };
  const win = {
    ...
    govmap: {
      ...
      getSearchResultData: () => (opts.fail500 && calls.rd++ < opts.fail500)
        ? Promise.reject(new Error('Request failed with status code 500'))
        : Promise.resolve(opts.farPolygon
...
(async () => {
  ...
  const one = [{ attributes: { fname: 'נווה שאנן', setl_name: 'חיפה' } }];
  const r1 = await run({ fail500: 2, layerFeatures: one });
