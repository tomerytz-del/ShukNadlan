...
## מה נבדק
  ...
  ‏6. **ודף ציבורי שיש בו קישור קשר — כן טוען אותו.** זה הכיוון ההפוך של
     כלל 5, והוא נולד מחור שנמצא בבדיקה הזו עצמה: היא אימתה סיווג רק
     בדפים ש**כבר** טענו את הסקריפט, ולכן דף ציבורי עם כפתור וואטסאפ
     ובלי הסקריפט עבר בשקט. כך תשעה דפים (‏about, terms, privacy, faq,
     ‏checkout ועוד) החזיקו 40 קישורי קשר שאיש לא ספר מהם קליק אחד.
     ‏**וזה הכשל השקט ביותר במשפחה:** הדף נטען, הכפתור עובד, הלקוח/ה
     מתקשר/ת — ובדוח פשוט אין שורה.
  ‏7. **אין פרטים אישיים באירוע.** ‏GA4 אוסר שליחת PII, וחשבון שנתפס
...
def check_page(path: Path) -> list[str]:
    ...
    # ‏הכיוון ההפוך, וראו כלל 6 בראש הקובץ: דף ציבורי עם קישור קשר שאינו
    # טוען את הסקריפט אינו "דף בלי מדידה" — הוא דף שהקליקים בו נעלמים.
    links = contact_links(text)
    if name not in PRIVATE_PAGES and links and not measured:
        problems.append(
            "דף ציבורי עם %d קישורי קשר שאינו טוען את assets/events.js —\n"
            "            אף קליק בו אינו נספר. השורה נכנסת מיד אחרי קטע ה-GTM:\n"
            '            <script src="assets/events.js"></script>' % len(links)
        )
