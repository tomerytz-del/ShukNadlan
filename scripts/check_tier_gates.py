...
GATES = {
    ...
    'לידי קונה/שוכר, פניות מהבאנרים ומהמחשבונים - מכסה אחת משותפת':
        'claim_lead',
    'לידי בעל-נכס מהפלטפורמה (מוכר/משכיר)':        'claim_lead',
    # שני הכלים שבדף המשרד ובדף הסוכן/ת. עד מיגרציה 20261230 הליד שלהם נכנס
    # ‏`unlocked` לכל המסלולים, כלומר השורה בטבלה הייתה ✓ בלי גייט — ובדיוק
    # לכיוון המסוכן יותר: Pay&GO קיבל/ה משם פניות ללא הגבלה וללא עלות.
    ...
    'מספר טלפון בתיאור המודעה ובביו האישי':        'paygo_text_rules_apply',
}
...
# ‏`claim_lead` יצאה מכאן: מיגרציה `20261230090000_lead_quota_pool_own_channel_25.sql`
# כותבת אותה במלואה, ולכן יש לה סוף-סוף קובץ מקומי. הרשימה התכווצה לריקה,
# וזה הכיוון היחיד שמותר לה.
LEGACY_GATES: set[str] = set()
