...
// משויך למנהל/ת המשרד, ובמסלול בתשלום נפתח מיד (‏status='unlocked') —
// אין מה לרכוש ואין מה לחכות לו. ב-Pay&GO הוא נכנס מוסתר ונספר במכסת
// הפניות החודשית, כי זה בדיוק מה שהמסלול הזה מוכר.
//
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // היוצא מן הכלל הוא ליד מדף משרד או מדף סוכן/ת: הוא לא נמכר לאיש, ולכן
    // במסלול בתשלום הוא נכנס פתוח ומי שהדף שייך לו/ה רואה/ה שם וטלפון מיד.
    //
    // **ב-Pay&GO הוא נכנס מוסתר**, ולא מפני שהוא שייך למישהו אחר אלא מפני
    // שהמסלול הזה נמכר כ"10 פניות חינם בחודש מכל המקורות יחד, ואז ₪25
    // לפנייה". ליד שנפתח כאן מעצמו היה עוקף את המונה — כלומר מקור אחד שבו
    // Pay&GO מקבל/ת פניות ללא הגבלה וללא עלות, בלי שאיש התכוון לכך ובלי
    // שמשהו היה מתלונן. הפתיחה עצמה, המכסה והמחיר יושבים ב-claim_lead.
    const paidTier = agencyRouting?.tier === "mid" || agencyRouting?.tier === "premium";
    const baseRow: Record<string, unknown> = {
      deal_type,
      agent_id: matchedAgentId,
      agency_id: matchedAgencyId,
      status: agencyRouting && paidTier ? "unlocked" : "masked",
      raw_name: name,
      raw_phone: phone,
    ...
    return json({
      ...
      note: agentRouting
        ? paidTier
          ...
          : "ליד מדף הסוכן/ת - שויך לסוכן/ת ונשמר מוסתר; הפתיחה נספרת במכסת הפניות החודשית"
        : agencyRouting
        ? paidTier
          ? "ליד מדף המשרד - שויך למנהל/ת המשרד ונפתח מיד, ללא עלות"
          : "ליד מדף המשרד - שויך למנהל/ת המשרד ונשמר מוסתר; הפתיחה נספרת במכסת הפניות החודשית"
        : matchedAgentId
