...
5. אין לכתוב שם של בעל/ת הנכס, מוכר/ת, שוכר/ת או דייר/ת - גם לא כשהשם
   מופיע בתיאור שהסוכן/ת כתב/ה. השמות היחידים שמותר לכתוב הם שם הסוכן/ת
   ושם המשרד.
6. עברית תקנית, גוף שלישי, ללא סימני קריאה כפולים וללא מילים כמו "מדהים",
   "חלומי", "הזדמנות שלא תחזור".
7. כשהנתונים דלים - לכתוב קצר. טקסט קצר ומדויק עדיף על טקסט ארוך ומנופח.
...
/** מקף ארוך יוצא מכל טקסט שמתפרסם - גם מזה שהמודל כתב.
 *
 *  **למה גם כאן וגם בפרומפט.** סעיף 8 בפרומפט מבקש מקף רגיל, ורוב הזמן
 *  זה עובד - אבל מקף ארוך הוא מה שמודל שפה מייצר כברירת מחדל בעברית
 *  ובאנגלית, והוא חוזר בדיוק כשהניסוח נעשה יפה יותר. הנחיה היא בקשה;
 *  זה מה שהופך אותה לערובה.
 *
 *  והטעות כאן שקטה במיוחד: התיאור נכתב פעם אחת, נשמר במסד, ומשם הוא
 *  זורם למודעה ולפוסט בפייסבוק. אף אחד לא קורא אותו לפני שהוא באוויר,
 *  ואין בדיקת CI שתתפוס טקסט שנוצר בזמן ריצה. */
export function noLongDash(s: string): string {
  return s.replace(/[\u2014\u2013\u2015]/g, "-");
}
...
export async function generateMarketingCopy(
  ...
  const description = noLongDash(String(parsed?.marketing_description ?? "").trim());
  const post = noLongDash(String(parsed?.post_text ?? "").trim());
