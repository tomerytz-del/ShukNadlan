 ...
 *
 * כמו ברשם החברות, הבדיקה היא על מה ש**פוסל** ולא על מה שמאשר: המאגר אינו
 * מחזיר דגל בוליאני אלא טקסט חופשי, וניסוח חדש שטרם ראינו צריך להיחשב
 * תקין. ההפך היה חוסם מתווך/ת מורשה/ית בגלל מילה שהמפרסם הוסיף.
 */
...
async function discoverFields(): Promise<Discovered | { error: string }> {
  ...
  discovered = {
    // "מספר" לבדו הוא רמז חלש (יש גם "מספר זהות"), ולכן הוא אחרון.
    number: pickField(ids, ["רישיון", "רשיון", "רשיונ", "תעודה", "מספר"]),
    name: pickField(ids, ["שם מלא", "שם המתווך", "שם"]),
 ...
 * ההצלבה מול השם שהוקלד הייתה נכשלת על חצי שם.
 */
