...
interface StyleDef {
   ...
   *
   * ‏`kitchen` יושב כאן אבל **אינו מיוצר** — ראו privateTargetsFor. הרשימה
   * היא קטלוג הסגנון ולא רשימת מה שרץ, ומי שיחזיר מטבח להשכרה צריך להחליט
   * את זה במקום אחד בלבד.
   */
 ...
 *
 * ‏**חוץ רק לבית פרטי**, בשני המצבים. בדירה בבניין החזית והחצר הן רכוש
 * משותף: במכירה הדמיית שיפוץ שלהן מציגה שדרוג שאיש לא מתחייב אליו, ובהשכרה
 * אין לשוכר/ת רשות להציב ריהוט בחצר שאינה שלו/ה. בבית פרטי החצר מושכרת עם
 * הבית, ולכן שם היא כן נכנסת — ומתמונת חצר בלבד (ראו STAGING_TARGET_ROOMS).
 *
 * ‏**חדר שינה רק בהשכרה, ורק אחד.** שוכר/ת בוחר/ת דירה לפי הסלון, החצר
 * וחדר השינה — ולכן החדר נכנס דווקא שם. אחד ולא יותר: חדר שינה שני ושלישי
 * הם אותה מיטה באותו סגנון בחדר קטן יותר, כלומר עוד קריאת Gemini לכל סגנון
 * בלי שום מידע חדש. ראו PRIVATE_TARGET_ROOMS לעדיפות יחידת ההורים.
 *
 * ‏**מטבח רק במכירה.** במטבח כמעט הכול מחובר — ארונות, משטח, חיפוי, כיור,
 * מכשירים — ובהלבשת בית אסור לגעת בכל אלה. מה שנשאר הוא כיסא בר, ראנר
 * וקערת פירות: קריאת Gemini בתשלום שמחזירה תמונה שכמעט זהה למקור, ו"אחרי"
 * שאינו נבדל מ"לפני" הוא ההוכחה הגרועה ביותר שאפשר להציג לכלי הזה.
 *
...
export function privateTargetsFor(propertyType: string | null, mode: RenderMode): PrivateTarget[] {
  ...
  if (hasOwnExterior(propertyType)) targets.push("exterior");
  targets.push("living_room");
  targets.push(mode === "staging" ? "bedroom" : "kitchen");
  return targets;
}
