...
// ============================================================================
// רכישת ליד ממדף ה-RSS — ₪50 מיתרת הארנק.
//
// אותה תבנית כמו lead-claim: זו פעולה שמעבירה כסף, ולכן verify_jwt=true
// והסוכן/ת הפועל/ת נגזר/ת מה-JWT המאומת בלבד — לעולם לא מה-body. פונקציית
// ה-DB ‏purchase_rss_lead נעולה ל-service_role, כך שאין דרך לעקוף את הנתיב
// הזה ולחייב סוכן/ת אחר/ת או לקבל את source_url בלי לשלם.
// ============================================================================
...
Deno.serve(async (req: Request) => {
  ...
  const { data: result, error: rpcErr } = await serviceClient.rpc("purchase_rss_lead", {
    p_lead_id: lead_id,
