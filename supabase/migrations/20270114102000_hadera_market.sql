...
--
-- חדרה, חריש, פרדס חנה-כרכור, אור עקיבא, בנימינה-גבעת עדה, קיסריה ושדות
-- ים, ו-24 יישובי המועצה האזורית מנשה. כ-289 אלף תושבים רשומים
-- (20270114096000). נולד **לא חי** - "נפתחים בקרוב" ב-noindex, לגיוס
...
--   - זכרון יעקב, דרום חוף הכרמל (מעגן מיכאל, דור, נחשולים ועוד) ואלונה -
--     שוק הזכרון, ייפתח או יצורף בהחלטה נפרדת.
...
--   - ‏slug אמיתי לשש הערים, רק כשהוא עדיין זמני. שדות ים ויישובי מנשה
--     נשארים עם הזמני.
-- ===========================================================================
...
begin
  select count(*) filter (where market_slug = 'hadera'),
    ...
    into v_in, v_other
    from public.cities
   where muni_code in ('6500','1247','7800','1020','9800','1167','327',
