...
# פינים זמניים: מודעות שמוצגות על כתובת צמודה כי הרחוב שלהן אינו בשכבת
# הכתובות של העירייה. **החוב נרשם כאן כדי שיהיה מי שיזכיר לסגור אותו.**
#
# ## למה רשימה מוצהרת ולא גלאי אוטומטי
#
# הניסיון הראשון כאן היה **גלאי כללי**: מודעה פעילה שהפין שלה זהה בדיוק
# לפין של מודעה בכתובת אחרת. הוא נבדק מול המסד לפני שנכתב, והוא נפל —
# מתוך חמש קבוצות שהוא מצא, **שלוש היו חיוביות-שווא**, וביניהן
# ‏`המלאכה` עצמו: הוא המקור של הפין ולא הלווה, ונתפס רק מפני שמישהו
# השאיל ממנו.
#
# **הגלאי סימטרי ואי אפשר לתקן זאת מהנתונים:** "מי השאיל ממי" אינו רשום
# בשום מקום, ו-`geocode_attempts` אינו מבדיל (הוא מתאפס בכל תיקון ידני).
# ממצא שיורה על המקור התקין הוא בדיוק ה"מדד שמודד משהו אחר ממה שהכותרת
# טוענת" שהסעיף *מדידה* ב-CLAUDE.md מתאר — והמחיר שלו גרוע מכלום, כי
# ממצא שיורה לשווא מלמד להתעלם מהפאנל.
#
# לכן ההשאלה **מוצהרת בכתב באותו PR שיוצר אותה**, כמו `PAUSED_WORKFLOWS`
# למעלה: שורה כאן היא חוב, והיא נושאת את הסיבה ואת התנאי לסגירה.
#
# ## מתי זה נסגר
#
# הסימן הוא שורת `street_registry`: כשסנכרון ה-GIS ימצא את הרחוב היא
# תהפוך ל-`source = 'gis'` עם `active = true`. אז הבדיקה
# `listings_stand_in_pin` פותחת ממצא, וההחזרה היא מחיקת הפין
# (`lat = null`) — התור פותר מחדש מהכתובת האמיתית.
#
# ‏`lat`/`lng` הם הערך **המדויק** שהמיגרציה כתבה, והם מה שמזהה את
# המודעות שעדיין נושאות את ההשאלה. מודעה שקיבלה בינתיים פין אחר אינה
# נספרת, וזה רצוי.
STAND_IN_PINS = (
    # (עיר, רחוב, lat, lng, מאיפה הושאל, המיגרציה)
    ("עפולה", "היצירה", 32.6018463, 35.29445, "המלאכה 25",
     "20261225090000_hayetzira_stand_in_pin.sql"),
)
